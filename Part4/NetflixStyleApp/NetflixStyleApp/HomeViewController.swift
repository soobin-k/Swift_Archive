...
class HomeViewController: UICollectionViewController{
    ...
    func getContents() -> [Content]{
        guard let path = Bundle.main.path(forResource: "Content", ofType: "plist"), // Content.plist의 경로
              let data = FileManager.default.contents(atPath: path), // path에 있는 data를 가져오기
              let list = try? PropertyListDecoder().decode([Content].self, from: data) else {return []} // data -> [Content]로 decode
        return list
    }
}
