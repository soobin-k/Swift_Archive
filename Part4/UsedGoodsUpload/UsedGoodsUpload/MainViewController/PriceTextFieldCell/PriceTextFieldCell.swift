...
class PriceTextFieldCell: UITableViewCell {
    ...
    func bind(_ viewModel: PriceTextFieldCellViewModel) {
        viewModel.showFreeShareButton
            .map{ !$0 } //반전 시킴
            .emit(to: freeShareButton.rx.isHidden) // 버튼 보임 제어
            .disposed(by: disposeBag)
        
        viewModel.resetPrice
            .map{ _ in ""}
            .emit(to: priceInputField.rx.text) // 텍스트 필드 초기화
            .disposed(by: disposeBag)
        
        priceInputField.rx.text // 텍스트 필드 입력 이벤트
            .bind(to: viewModel.priceValue)
            .disposed(by: disposeBag)
        
        freeShareButton.rx.tap
            .bind(to: viewModel.freeShareButtonTapped) // 버튼 클릭 이벤트
            .disposed(by: disposeBag)
    }
